...
def log_batch_info(batch, name):
    logging.info(f'{name} - Batch size: {len(batch)}')
    for tensor in batch:
        logging.info(f'{name} - Tensor shape: {tensor.shape}')
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.num_epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            log_batch_info(batch, "Train Loader")
            if D_step>0:
...
def train(args,lens, model,batch_idx,batch,Discriminator, optimizer, epoch, training_step, writer):
    ...
    log_batch_info(batch, "Train Batch")
    (
        obs_traj,
        pred_traj_gt,
        obs_traj_rel,
...
def D_train(args,lens, model,batch_idx,batch,Discriminator, optimizer, epoch, training_step, writer):
    ...
    model.train()
    batch = [tensor.cuda() for tensor in batch]
    log_batch_info(batch, "D Train Batch")
    (
        obs_traj,
        pred_traj_gt,
        obs_traj_rel,
...
def validate(args, model, val_loader, epoch, writer):
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            log_batch_info(batch, "Validation Batch")
            batch = [tensor.cuda() for tensor in batch]
            (
                obs_traj,
                pred_traj_gt,
