#changed normalization layer from batch norm to self norm.
#
import torch
...
class GAT(nn.Module):
    ...
    def forward(self, x, Relation):
        ...
        for i, gat_layer in enumerate(self.layer_stack):
            x = self.norm_list[i](x.reshape(bs, x.size(2), -1)).reshape(bs, n, -1)
            x, attn = gat_layer(x, Relation)
            if i + 1 == self.n_layer:
                x = x.squeeze(dim=1)
            else:
                x = F.elu(x.transpose(1, 2).contiguous().reshape(bs, n, -1))
...
class TrajectoryGenerator(nn.Module):
    ...
    def get_next_state(self, pred_traj_rel, obs_traj_pos, pred_state):
        ...
        real_pos = relative_to_abs(pred_traj_rel, start_pos)   # T, V, C
        ...
        with torch.no_grad():
            for j in range(self.obs_len):
                if j <= kl:
                    ...
                    staend[0, 1] = kl + 1
                    graph_inter_input = self.seqgatencoder(graph_lstm_input[(j - kl):(j + 1)].permute(1, 0, 2), staend)
...
class TrajectoryDiscriminator(nn.Module):
    ...
    def forward(self, traj, state, seq_start_end):
        ...
        for i, input_t in enumerate(state[:].chunk(state[:].size(0), dim=0)):
            ...
            state_lstm_h_t, state_lstm_c_t = self.state_part_lstm(
