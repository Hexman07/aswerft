...
def plot_trajectory(args, loader, generator):
    ...
    print("Starting plot_trajectory function.")
    
    with torch.no_grad():
        for batch in loader:
            batch = [tensor.cuda() for tensor in batch]
            print("Batch tensors moved to GPU.")
            ...
            if len(batch) == 9:
                ...
                raise ValueError(f"Expected 9 elements in batch but got {len(batch)}")
            ...
            print("pred_traj_gt_rel shape:", pred_traj_gt_rel.shape)
            print("non_linear_ped shape:", non_linear_ped.shape)
            print("loss_mask shape:", loss_mask.shape)
            print("seq_start_end shape:", seq_start_end.shape)
            print("extra_tensor_1 shape:", extra_tensor_1.shape)
            print("extra_tensor_2 shape:", extra_tensor_2.shape)
            
            ade = []
            
            # Debug statements to check seq_start_end
            print(f'seq_start_end type: {type(seq_start_end)}')
            print(f'seq_start_end content: {seq_start_end}')
        
            # Ensure seq_start_end is a tensor
            if not isinstance(seq_start_end, torch.Tensor):
                seq_start_end = torch.tensor(seq_start_end, device=args.device)
        
            # Ensure seq_start_end is properly formed
            print(f'seq_start_end shape: {seq_start_end.shape}')

            # Check if seq_start_end is not empty
            if seq_start_end.shape[0] == 0:
                print("seq_start_end is empty, skipping this batch.")
                continue
    ...
    print("Finished plot_trajectory function.")
